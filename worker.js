export default { async fetch(request, env) {
  ...
  const response = await fetch("https://api.302.ai/v1/chat/completions", {method:"POST", headers:{"Authorization":`Bearer ${env.OPENAI_API_KEY}`,"Content-Type":"application/json"}, body:JSON.stringify({model:"gpt-5.5",temperature:.2,messages:[{role:"system",content:system},{role:"user",content:JSON.stringify({mode:body.mode||"task_extract",today:new Date().toISOString().slice(0,10),message:body.message||"",context:body.context||{}})}]})});
