...
const KEY="pw_v04_data", CRED_KEY="pw_v03_credential_id";
...
$$("[data-action]").forEach(b=>b.onclick=()=>{const m={addWork:"work",addExpense:"expense",addLife:"life",addStudy:"study",addRadar:"radar",addInbox:"inbox"};openForm(m[b.dataset.action]);});
...
$("#saveSheet").onclick=()=>{
  ...
  if(currentForm==="radar")data.radar.unshift(obj);
...
function renderConfirm(){
  $("#confirmArea").innerHTML=smartParsed.map((x,i)=>{
    ...
    if(x.kind==="radar") fields=confirmField("事项/商机","title",x.title)+confirmField("预计发生时间","expectedDate",x.expectedDate||x.date,"date")+confirmField("下次跟进日期","date",x.date,"date")+confirmField("阶段","horizon",x.horizon,"select",["7天内","30天内","90天内","90天+"])+confirmField("备注","note",x.note,"textarea");
...
$("#saveSmart").onclick=()=>{
  const cards=$$(".confirm-card");
  cards.forEach((card,i)=>{
    const obj={id:Date.now()+i,raw:smartParsed[i].raw};
    ...
    if(kind==="work")data.work.unshift(obj);
    if(kind==="life")data.life.unshift(obj);
    if(kind==="study")data.study.unshift(obj);
    if(kind==="radar")data.radar.unshift(obj);
